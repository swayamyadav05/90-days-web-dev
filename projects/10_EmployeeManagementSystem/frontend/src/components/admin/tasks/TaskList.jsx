...
const TaskList = ({
  ...
  const handleStatusUpdate = async (taskId, newStatus) => {
    try {
      ...
      if (result.success) {
        // If status filter is active and new status doesn't match, clear the filter
        if (statusFilter !== "all" && statusFilter !== newStatus) {
          setStatusFilter("all");
        }

        // Call parent callback with task ID and updates
  ...
  const handlePriorityUpdate = async (taskId, newPriority) => {
    try {
      ...
      if (result.success) {
        // If priority filter is active and new priority doesn't match, clear the filter
        if (
          priorityFilter !== "all" &&
